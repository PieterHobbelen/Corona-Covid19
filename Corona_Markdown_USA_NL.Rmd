...
corona_usa_total_state %>% ggplot(aes(population_states_USA/10^6, confirmed_cases_USA/10^3, label = abbs_states_USA)) + geom_label() + geom_smooth(method = "lm") + ggtitle("Figure a; Corona patients in the 50 states, DC and the American Territories") + ylab("Confirmed cases * 1.000") + xlab("Population of the state * 1.000.000")

corona_usa_total_state %>% ggplot(aes(population_states_USA/10^6, confirmed_cases_USA/10^3, label = abbs_states_USA)) + geom_smooth(method = "gam") + geom_label_repel() + geom_point() + ggtitle("Figure b; Corona patients in the 50 states, DC and the American Territories") + ylab("Confirmed cases * 1.000") + xlab("Population of the state * 1.000.000, log 10 scale") + scale_x_continuous(trans = "log10")


```

When we look at the first figure and the ouliers with an higher confirmed rate than usual, it looks like that the majority of the outliers are located in the North East region. When we look at the second figure see the actual outliers, there are 7 outliers with a higher rate. Connecticut, Massachusetts, New Jersey, New York, Maryland are outliers in the region North East and the other two are Louisiana in the South Mid region and Illinois in the Mid region.

```{r plots about the USA death cases states scatterplot, echo=FALSE, message= FALSE}
corona_usa_total_state %>% ggplot(aes(population_states_USA/10^6, death_cases_USA, label = abbs_states_USA, color = region_USA)) + geom_label() + geom_abline(intercept = 0, slope = (death_cases_total_USA)/(population_USA/10^6)) + ggtitle("Figure c; Corona patients in the 50 states, DC and the American Territories") + ylab("Death cases") + xlab("Population of the state * 1.000.000")

corona_usa_total_state %>% ggplot(aes(population_states_USA/10^6, death_cases_USA, label = abbs_states_USA)) + geom_smooth(aes(population_states_USA/10^6, death_cases_USA)) +  geom_label_repel() + geom_point() + ggtitle("Figure d; Deaths due to Corona in the 50 states, DC and the American Territories") + ylab("Death cases") + xlab("Population of the state * 1.000.000, log 10 scale") + scale_x_continuous(trans = "log10")
```
...
In the next 6 figures we do the same thing, but instead of the data for the United States and the American Territories, we use the data for The Netherlands. The RIVM, the official publisher for the Corona data for The Netherlands, provides also the data about the confirmed cases and the death cases, but they provide also data about the hospital cases (the Intensive Care patients). In The Netherlands there are instead of 50 states 12 provinces.
We see again in the first figure 12 points represinting the provinces and a black line representing the average for The Netherlands. We that there are almost no outliers, maybe Noord-Brabant and Limburg. When we look at the next figure, we see again a line, but this time also an area that indicates the area where a 'normal' province is located. We see that Noord- Brabant is located on the border and has a slightly higher confirmed rate, and Noord-Holland is also located on the border and has a small slightly lower confirmed rate.

```{r plots about the NL confirmed cases states scatterplot, echo=FALSE, message= FALSE}
corona_nl_total_province %>% ggplot(aes(population_provices_NL/10^6, confirmed_cases_NL, label = province_NL)) + geom_smooth(method = "lm") + ggtitle("Figure e; Corona patients in the 12 provinces in The Netherlands") + ylab("Confirmed cases") + xlab("Population of the province * 1.000.000") + geom_label_repel() + geom_point()


corona_nl_total_province %>% ggplot(aes(population_provices_NL/10^6, confirmed_cases_NL, label = province_NL)) +geom_smooth(method = "gam") + geom_label_repel() + scale_x_continuous(trans = "log10") + geom_point() + ggtitle("Figure f; Corona patients in the 12 provinces in The Netherlands") + ylab("Confirmed cases") + xlab("Population of the province * 1.000.000, log 10 scale") + scale_x_continuous(trans = "log10")
```

We see again the same figures, the amount of deaths per state in a continious X-scale and in a logarithmic X-scale. When we compare the locations of the provinces in the figure with the locations of the provinces in the figures with the confirmed cases (figures e and f) we see that Noord-Brabant further away from the avarage line, compared to the changes of the other provinces. So, the ratio that indicates the probability of how many patients died due to Corona, given that the patient has Corona, is high in Noord-Brabant compared to the other provinces. Another thing that we see is that Gelderland is below the average line in the death cases figure, and located on the line in the confirmed cases figure. So the probability that a patient died due to Corona, given that the patient has Corona, is low in Gelderland.

```{r plots about the NL death cases states scatterplot, echo=FALSE, message= FALSE}
corona_nl_total_province %>% ggplot(aes(population_provices_NL/10^6, death_cases_NL, label = province_NL)) + geom_smooth(method = "lm") + geom_point(aes(color = region_NL)) + ggtitle("Figure g; Corona patients in the 12 provinces in The Netherlands") + ylab("Death cases") + xlab("Population of the province * 1.000.000") + geom_label_repel(aes(color = region_NL)) + theme(legend.position = "right", legend.background = element_rect(fill="lightblue", size=0.5, linetype = "solid", color = "darkblue")) + labs(color = "Regions The Netherlands: ")

corona_nl_total_province %>% ggplot(aes(population_provices_NL/10^6, death_cases_NL, label = province_NL))  + geom_smooth(method = "gam") + geom_label_repel(aes(color = region_NL)) + scale_x_continuous(trans = "log10") + geom_point(aes(color = region_NL)) + ggtitle("Figure d; Deaths due to Corona in the 12 provinces in The Netherlands") + ylab("Death cases") + xlab("Population of the state * 1.000.000, log 10 scale") + scale_x_continuous(trans = "log10") + theme(legend.position = "right", legend.background = element_rect(fill="lightblue", size=0.5, linetype = "solid", color = "darkblue")) + labs(color = "Regions The Netherlands: ")
```


```{r plots about the NL hospital cases states scatterplot, echo=FALSE, message= FALSE}
corona_nl_total_province %>% ggplot(aes(population_provices_NL, hospital_cases_NL, label = province_NL)) + geom_smooth(method = "lm") + geom_label_repel(aes(color = region_NL)) + geom_point(aes(color = region_NL)) + theme(legend.position = "right", legend.background = element_rect(fill="lightblue", size=0.5, linetype = "solid", color = "darkblue")) + labs(color = "Regions The Netherlands: ")

corona_nl_total_province %>% ggplot(aes(population_provices_NL, hospital_cases_NL, label = province_NL)) + geom_smooth(method = "gam") + scale_x_continuous(trans = "log10") + geom_label_repel(aes(color = region_NL)) + geom_point(aes(color = region_NL)) + theme(legend.position = "right", legend.background = element_rect(fill="lightblue", size=0.5, linetype = "solid", color = "darkblue")) + labs(color = "Regions The Netherlands: ")
```
